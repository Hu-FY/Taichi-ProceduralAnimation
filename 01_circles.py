...
@ti.kernel
def circle(radius: ti.f32, blur: ti.f32, tiles: ti.i32, iterations: ti.i32):
    clear()
    size = res_x/tiles
    for t in ti.static(range(3)):
        ...
        for i, j in canvas:
            a = iterations *0.03
            r = (0.5 * ti.sin(a + float(i) / res_x + t) + 0.5)/(t+1)
            g = (0.5 * ti.sin(a + float(j) / res_y + 2 + t) + 0.5)/(t+1)
            b = (0.5 * ti.sin(a + float(i) / res_x + 4 + t) + 0.5)/(t+1)
...
while gui.running:
    ...
    if r < 0: r = -r
    circle(r*tilesize/2, blur, tiles, iterations)
    #test_inter()
